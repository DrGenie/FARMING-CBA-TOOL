...
(function () {
  function init() {
    ...
    for (var iTab = 0; iTab < tabButtons.length; iTab++) {
      tabButtons[iTab].addEventListener("click", function (e) {
    ...
    function updateBaseFromInputs() {
      ...
      state.base.discountRatePct = Math.max(
        0,
        toNumberOrZero(inputDiscount.value)
      );
      state.base.defaultPricePerTonne = Math.max(
        0,
        toNumberOrZero(inputPrice.value)
      );
    }
    ...
    function renderTreatmentsTable() {
      ...
      for (var i = 0; i < state.treatments.length; i++) {
        ...
        tr.innerHTML =
          ...
          '<input type="checkbox" data-field="isControl"' +
          (t.isControl ? " checked" : "") +
          " />" +
          "</td>" +
          '<td><input type="number" step="0.01" data-field="yieldTPerHa" value="' +
          t.yieldTPerHa +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="pricePerTonne" value="' +
          (t.pricePerTonne === "" ? "" : t.pricePerTonne) +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="capitalCostPerHa" value="' +
          t.capitalCostPerHa +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="costSeedPerHa" value="' +
          t.costSeedPerHa +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="costChemPerHa" value="' +
          t.costChemPerHa +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="costLabourPerHa" value="' +
          t.costLabourPerHa +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="costMachPerHa" value="' +
          t.costMachPerHa +
          '" /></td>' +
          '<td><input type="number" step="1" data-field="costOtherPerHa" value="' +
          t.costOtherPerHa +
          '" /></td>' +
          '<td class="total-cost-cell">' +
          formatMoney(totalAnnual) +
          "</td>" +
          '<td><input type="number" step="1" data-field="extraBenefitsPerHa" value="' +
          t.extraBenefitsPerHa +
          '" /></td>' +
          '<td class="actions-cell">' +
          '<button type="button" class="btn btn-ghost btn-small" data-action="remove" data-index="' +
          i +
          '">✕</button>' +
          "</td>";
        ...
        try {
          if (typeof XLSX === "undefined") {
            uploadStatus.textContent =
              "Excel library is not available. Check your internet connection or the CDN script.";
            return;
          }
          ...
          console.error(err);
          uploadStatus.textContent =
            "There was a problem reading this file. Please check that it is a valid Excel file.";
        } finally {
          inputUpload.value = "";
        }
      };
    ...
    function computeResults() {
      ...
      for (var i = 0; i < state.treatments.length; i++) {
        ...
        var price =
          t.pricePerTonne === "" ||
          t.pricePerTonne === null ||
          t.pricePerTonne === undefined
            ? defaultPrice
            : toNumberOrZero(t.pricePerTonne);

        var annualGrossBenefitPerHa =
          t.yieldTPerHa * price + t.extraBenefitsPerHa;
        var annualCostPerHa =
          t.costSeedPerHa +
          t.costChemPerHa +
          t.costLabourPerHa +
    ...
    function buildAiPrompt(computed) {
      ...
      lines.push(
        "- When a treatment has a low BCR or negative NPV, suggest realistic ways performance could improve (for example reduce costs, increase yield, improve prices or change agronomic practices), framed as options rather than instructions."
      );
      lines.push("");
      ...
      lines.push(
        "- Default grain price where not specified: $" +
          state.base.defaultPricePerTonne +
          " per tonne"
      );
      lines.push("");
      ...
      for (var j = 0; j < sorted.length; j++) {
        ...
        lines.push(
          "- NPV per ha relative to control: " +
            formatMoney(r.diffNpvPerHa)
        );
        if (
          r.bcr !== null &&
          r.bcr !== undefined &&
          ...
          lines.push(
            "- BCR: not defined (PV costs are zero or missing)"
          );
        }
        if (
          r.roi !== null &&
          ...
          lines.push(
            "- ROI: not defined (PV costs are zero or missing)"
          );
        }
        lines.push(
          "- Whole-farm NPV over the horizon: " + formatMoney(r.npvFarm)
        );
      }

      lines.push("");
      lines.push(
        "Your task is to write a two to three page interpretation (around 1200–1800 words). Focus on:"
      );
      lines.push(
        "1. Explaining in simple terms what drives PV benefits and PV costs for these treatments (yields, prices, capital costs, annual variable costs and any extra benefits)."
      );
      lines.push(
        "2. Comparing each treatment to the control and highlighting where it gains or loses economically and why."
      );
      lines.push(
        "3. For treatments with weak performance (low BCR or negative NPV), discussing practical improvement options such as reducing particular cost components, improving agronomy to lift yields or targeting better prices."
      );
      lines.push(
        "4. Emphasising uncertainty and that results depend on the assumptions. Encourage the farmer to test alternative prices, yields and cost structures rather than treating any single treatment as the automatic answer."
      );
      lines.push(
        "5. Keeping the tone supportive and exploratory. The goal is to help the farmer understand the numbers and think through what they might want to change, not to instruct them."
      );
    ...
    btnCopyPrompt.addEventListener("click", function () {
      try {
        ...
        alert(
          "Copy to clipboard may not be available. Please select and copy manually."
        );
      }
    });

    btnDownloadPrompt.addEventListener("click", function () {
      var blob = new Blob([aiPromptTextarea.value], {
    ...
    btnExportExcel.addEventListener("click", function () {
      if (typeof XLSX === "undefined") {
        alert(
          "Excel export is not available. Check that the XLSX library is loaded from the CDN."
        );
        return;
      }
